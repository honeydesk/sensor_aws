...
with DAG(
    ...
    schedule="@weekly",
    start_date=pendulum.datetime(2023, 3, 19, tz="UTC"),
    catchup=False,
    
    tags=["ml","classification","sensor"],
) as dag:
    # [END instantiate_dag]
    # [START documentation]
    dag.doc_md = __doc__
    ...
    def data_transformation(**kwargs):
        ti = kwargs["ti"]
        from sensor.entity.artifact_entity import DataIngestionArtifact
        data_ingestion_artifact = ti.xcom_pull(task_ids="data_validation",key="data_ingestion_artifact")
        data_ingestion_artifact = DataIngestionArtifact(**(data_ingestion_artifact))
    ...
    def model_evaluation(**kwargs):
        ti = kwargs["ti"]
        from sensor.entity.artifact_entity import DataIngestionArtifact,DataTransformationArtifact,ModelTrainerArtifact
        data_transformation_artifact = ti.xcom_pull(task_ids="data_transformation",key="data_transformation_artifact")
        data_transformation_artifact = DataTransformationArtifact(**(data_transformation_artifact))
    ...
    def model_pusher(**kwargs):
        ti = kwargs["ti"]
        from sensor.entity.artifact_entity import DataTransformationArtifact,DataIngestionArtifact,DataValidationArtifact,ModelTrainerArtifact
        data_transformation_artifact = ti.xcom_pull(task_ids="data_transformation",key="data_transformation_artifact")
        data_transformation_artifact = DataTransformationArtifact(**(data_transformation_artifact))
